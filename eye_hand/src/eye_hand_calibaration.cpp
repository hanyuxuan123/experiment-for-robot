...
int main(int argc, char** argv){
  ...
  ros::NodeHandle node;
  	...
  	ofstream outfile_1("out_robot.txt", ios::trunc);
    ofstream outfile_2("out_kinect.txt", ios::trunc);
...
//  * @param Hcij [in] 4x4xN matrix:The transformation between the cameras.
  ...
  try
        ...
        //得到坐标kinect2和坐标标定板之间的关系
             listener.waitForTransform("kinect2_rgb_optical_frame", "camera_marker", ros::Time(0), ros::Duration(3.0));
             listener.lookupTransform("kinect2_rgb_optical_frame", "camera_marker", ros::Time(0), transform);
            }
...
//构造机器人基坐标系和机器人末端坐标系之间的关系矩阵
        ...
        // a1=1-2*pow(robot_oriation_y1,2)-2*pow(robot_oriation_z1,2);
        // a2=2*robot_oriation_x1*robot_oriation_y1-2*robot_oriation_z1*robot_oriation_w1;
        // a3=2*robot_oriation_x1*robot_oriation_z1+2*robot_oriation_y1*robot_oriation_w1;
        // a4=2*robot_oriation_x1*robot_oriation_y1+2*robot_oriation_z1*robot_oriation_w1;
        // a5=1-2*pow(robot_oriation_x1,2)-2*pow(robot_oriation_z1,2);
        // a6=2*robot_oriation_z1*robot_oriation_y1-2*robot_oriation_x1*robot_oriation_w1;
        // a7=2*robot_oriation_x1*robot_oriation_z1-2*robot_oriation_y1*robot_oriation_w1;
        // a8=2*robot_oriation_z1*robot_oriation_y1+2*robot_oriation_x1*robot_oriation_w1;
        // a9=1-2*pow(robot_oriation_y1,2)-2*pow(robot_oriation_x1,2);
