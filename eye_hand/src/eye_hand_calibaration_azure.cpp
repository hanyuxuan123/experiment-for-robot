...
//  * @param Hcij [in] 4x4xN matrix:The transformation between the cameras.
  ...
  try
        ...
        //得到坐标kinect2和坐标标定板之间的关系
             listener.waitForTransform("rgb_camera_link", "camera_base", ros::Time(0), ros::Duration(3.0));
             listener.lookupTransform("rgb_camera_link", "camera_base", ros::Time(0), transform);
            }
